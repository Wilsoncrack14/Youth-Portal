    ...
    <div className="p-4 lg:p-10 animate-fade-in-up">
      <div className="max-w-6xl mx-auto flex flex-col gap-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
          ...
          <div>
            <h1 className="text-4xl font-black tracking-tight text-gray-900 dark:text-white mb-2">Mi Perfil</h1>
            <p className="text-gray-500 dark:text-slate-400 text-lg">Sigue tu progreso espiritual y alcanza nuevas metas.</p>
          </div>
          <button onClick={handleShareProgress} className="bg-white dark:bg-[#1e1e2d]/60 hover:bg-gray-50 dark:hover:bg-primary/20 transition-all text-gray-900 dark:text-white px-5 py-2.5 rounded-lg flex items-center gap-2 font-medium border border-gray-200 dark:border-white/10 shadow-lg">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 bg-white dark:bg-[#1a1b26] border border-gray-200 dark:border-white/5 shadow-sm dark:shadow-none rounded-2xl p-6 flex flex-col items-center text-center relative overflow-hidden">
            ...
            <div className="relative z-10 mt-4 mb-4">
              <div className="w-32 h-32 rounded-full p-1 bg-gradient-to-tr from-primary to-purple-500 shadow-2xl">
                <div className="w-full h-full rounded-full border-4 border-white dark:border-[#1e1e2e] bg-cover bg-center overflow-hidden"
            ...
            {isEditingUsername ? (
              ...
              <div className="flex items-center gap-2">
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{profile?.username || 'Usuario'}</h2>
                <button
            ...
            <div className="w-full mt-auto bg-gray-50 dark:bg-slate-800/50 rounded-xl p-4 border border-gray-200 dark:border-white/5">
              <div className="flex justify-between items-end mb-2">
                <span className="text-xl font-bold text-gray-900 dark:text-white">Nivel {stats.level}</span>
              ...
              <p className="text-xs text-gray-500 dark:text-slate-400 mt-3 text-left">Lee 5 capítulos más para el Nivel {stats.level + 1}</p>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <div className="bg-white dark:bg-[#1a1b26] border border-gray-200 dark:border-white/5 shadow-sm dark:shadow-none rounded-xl p-6 flex flex-col sm:flex-row gap-6 items-center border-l-4 border-l-primary">
              <div className="bg-primary/10 dark:bg-primary/20 p-3 rounded-full text-primary shrink-0"><span className="material-symbols-outlined">lightbulb</span></div>
              <div className="flex-1">
                <h4 className="text-gray-900 dark:text-white font-bold text-lg">Versículo del Día</h4>
                {dailyVerse ? (
                  <p className="text-gray-600 dark:text-slate-300 italic text-sm">"{dailyVerse.text}" - {dailyVerse.reference}</p>
                ) : (
                  <p className="text-gray-600 dark:text-slate-300 italic text-sm">Cargando versículo...</p>
                )}
              </div>
              {dailyVerse && (
                <button
        ...
        <section className="pb-20">
          <div className="flex justify-between items-center mb-6">
            ...
            <span className="text-gray-500 dark:text-slate-400 text-sm">{badges.filter(b => b.unlocked).length} de {badges.length} Desbloqueados</span>
          </div>
