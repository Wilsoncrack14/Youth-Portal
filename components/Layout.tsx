...
const Layout: React.FC<LayoutProps> = ({ children, activeScreen, setActiveScreen, userStats }) => {
  const menuItems = [
    { id: 'dashboard', icon: 'dashboard', label: 'Dashboard' },
    { id: 'studies', icon: 'menu_book', label: 'Mis Estudios' },
    ...
    <div className="flex h-screen w-full overflow-hidden">
      ...
      <aside className="hidden lg:flex w-72 flex-col justify-between bg-[#0e0e15] border-r border-[#292938] p-6 shrink-0 h-full overflow-y-auto">
        <div className="flex flex-col gap-8">
          <div className="flex items-center gap-3 px-2 cursor-pointer" onClick={() => setActiveScreen('dashboard')}>
          ...
          <div className="glass-panel p-4 rounded-xl flex items-center gap-4 cursor-pointer hover:bg-white/5 transition-colors" onClick={() => setActiveScreen('profile')}>
          ...
          <nav className="flex flex-col gap-2">
            {menuItems.map((item) => (
              ...
              >
                <span className={`material-symbols-outlined ${activeScreen === item.id ? 'fill-1' : ''}`}>{item.icon}</span>
        ...
        <div className="flex flex-col gap-2">
          <button 
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${
              activeScreen === 'settings' ? 'bg-white/10 text-white' : 'text-gray-400 hover:bg-[#292938] hover:text-white'
            }`}
          >
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-background-light dark:bg-background-dark relative h-screen">
        ...
        <header className="h-20 flex items-center justify-between px-8 border-b border-[#292938] bg-background-light/50 dark:bg-background-dark/50 backdrop-blur-md sticky top-0 z-20">
          <div className="lg:hidden flex items-center gap-3 text-white">
            <span className="material-symbols-outlined" onClick={() => setActiveScreen('dashboard')}>menu</span>
          ...
          <div className="flex items-center gap-4">
            <button className="relative p-2 rounded-full hover:bg-[#292938] text-gray-400 hover:text-white transition-colors" onClick={() => setActiveScreen('community')}>
